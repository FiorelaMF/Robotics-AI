...
syms q1 q2 q3 q4 l1 l2 l3 l4 l5 q1p q2p q3p q4p pi
% a d al th
DH = [ q1 l1 l2 0
       q2 0 l3 0
       0 q3 0 0
...
A1 = matra(DH(1,1),DH(1,2),DH(1,3),DH(1,4));
A2 = matra(DH(2,1),DH(2,2),DH(2,3),DH(2,4));
A3 = matra(DH(3,1),DH(3,2)/2,DH(3,3),DH(3,4));
T3 = A1*A2*A3;
x3 = T3(1,4);
y3 = T3(2,4);
z3 = T3(3,4);
...
A2 = matra(DH(2,1),DH(2,2),DH(2,3),DH(2,4));
A3 = matra(DH(3,1),DH(3,2)/2,DH(3,3),DH(3,4));
A4 = matra(DH(4,1),DH(4,2)/2,DH(4,3),DH(4,4));
T4 = A1*A2*A3*A4;
x4 = T4(1,4);
y4 = T4(2,4);
z4 = T4(3,4);
% Derivamos con respecto al tiempo
x4p = diff(x4,'q1')*q1p+diff(x4,'q2')*q2p+diff(x4,'q3')*q3p +diff(x4,'q4')*q4p;
y4p = diff(y4,'q1')*q1p+diff(y4,'q2')*q2p+diff(y4,'q3')*q3p +diff(y4,'q4')*q4p;
z4p = diff(z4,'q1')*q1p+diff(z4,'q2')*q2p+diff(z4,'q3')*q3p +diff(z4,'q4')*q4p;
% Elevamos al cuadrado
...
syms m1 m2 m3 m4% Masas
% Energía Cinética del Sistema
% K = K1 + K2 ---> Ki = 1/2*mi*vi^2
K1 = 1/2*m1*v12; K2 = 1/2*m2*v22;
K3 = 1/2*m3*v32; K4 = 1/2*m4*v42;
K = K1+K2+K3+K4;
% Matriz de Inercias
...
c34 = c34 + 1/2*(diff(m34,'q3')+diff(m34,'q4')-diff(m44,'q3'))*q4p;
c34 = simplify(c34);
...
% c44
c44 =       1/2*(diff(m44,'q1')+diff(m41,'q3')-diff(m14,'q4'))*q1p;
c44 = c44 + 1/2*(diff(m44,'q2')+diff(m42,'q3')-diff(m24,'q4'))*q2p;
c44 = c44 + 1/2*(diff(m44,'q3')+diff(m43,'q3')-diff(m34,'q4'))*q3p;
c44 = c44 + 1/2*(diff(m44,'q4')+diff(m44,'q2')-diff(m44,'q4'))*q4p;
c44 = simplify(c44);
