...
t0 = 0;
dt = 0.00001; % Muy pequeño para simular continuidad
tf = 5;
x = [ 0.1 0 0 0 0 0 ]';
k = 1;
...
Kv = 200*eye(2); lam = 10*eye(2); gam = 10*eye(2);
kp = 100; kv = 20;
...
% Calculo de la Matriz de Regresión y de la Señal de Control
f = qdpp + lam*ep;
fa = qdp + lam*e;
W(1,1) = a1^2*(cos(x(2)))^2*f(1)-0.5*a1^2*sin(2*x(2))*x(4)*fa(1) ...
-0.5*a1^2*sin(2*x(2))*x(3)*fa(2);
W(1,2) = 0;
W(2,1) = 0;
W(2,2) = a1^2*f(2)+0.5*a1^2*sin(2*x(2))*x(3)*fa(1)+a1*g*cos(x(2));
tau = Kv*r + W*[x(5) x(6)]';
...
title('Control Adaptivo: Estimación de Masa')
xlabel('Segundos'); ylabel('Masas')
grid on; zoom on
